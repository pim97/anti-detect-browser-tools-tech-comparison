...
web-data API. Scrappey is a paid substitute for the tools compared here, so the sponsor
has a commercial interest in how these tools are described.
...
Tier D indicates absent evidence, not a negative finding.
...
No benchmark was run. What follows is the provenance of each project's own claims.
